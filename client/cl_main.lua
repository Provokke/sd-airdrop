...
RegisterNetEvent("sd-airdrop:crate:createQbTarget", function(netId, crate, item, amount)
    if not netId or netId == 0 then
        print("[sd-airdrop] Error: Invalid netId in createQbTarget")
        return
    end
    
    if not item or not amount or amount <= 0 then
        print("[sd-airdrop] Error: Invalid item or amount in createQbTarget")
        return
    end
    ...
    -- Wait until the entity exists on the client with timeout
    while not DoesEntityExist(entity) and attempts < 200 do
        Wait(50)
    ...
    if not DoesEntityExist(entity) then
        print("[sd-airdrop] Error: Entity does not exist after 200 attempts for netId: " .. tostring(netId))
        return
    end
    
    -- Safely add target with error handling
    ...
    if not success then
        print("[sd-airdrop] Error adding target to entity: " .. tostring(err))
    end
end)
...
RegisterNetEvent("sd-airdrop:client:StartDrop", function(item, amount, roofCheck, planeSpawnDistance, dropCoords)
    CreateThread(function()          
        if dropCoords.x and dropCoords.y and dropCoords.z and tonumber(dropCoords.x) and tonumber(dropCoords.y) and tonumber(dropCoords.z) then            
            ...
            dropCoords = vector3(0.0, 0.0, 72.0)  -- Fixed: Use proper vector3 constructor
        end
        RequestWeaponAsset(GetHashKey(Config.FlareName))
        while not HasWeaponAssetLoaded(GetHashKey(Config.FlareName)) do
            Wait(0)
